# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## What This Repo Is

A 52-card deck goal tracking system. Each card is a markdown file representing a personal learning or development goal. This is a **content-only repo** — no build system, no tests, no code to run. All work is reading and editing `.md` files.

## Repo Layout

- `cards/` — The 54 card files organized by suit subdirectories (`clubs/`, `diamonds/`, `hearts/`, `spades/`) plus two joker files at the cards root
- `CARD_TEMPLATE.md` — Canonical template; all card files must follow this structure
- `DECK_OVERVIEW.md` — Quick-reference table of all 52 goals
- `PROGRESS.md` — Completion counts, suit/tier breakdowns, milestones checklist
- `CHANGELOG.md` — Chronological log of all updates (keep-a-changelog style with Added/Changed/Completed/Removed/Notes sections)
- `BACKLOG.md` / `RESOURCES.md` / `GOALS_TLDR.md` — Supporting reference docs
- `REFLECTIONS.md` — Standing log for cross-cutting, system-level, or product-level insights that don't belong to any single card's lifecycle (distinct from each card's own Reflection & Lessons Learned section)
- `WORKFLOW.md` — Describes the deck's relationship to the external daily to-do list (deck = macro/strategic, list = micro/daily-resolution); the list itself is not tracked in this repo

## Card File Conventions

Card filenames: `{value}-{suit}.md` (e.g., `02-spades.md`, `king-hearts.md`, `ace-diamonds.md`). Face cards use full names (`jack`, `queen`, `king`, `ace`).

Every card has these sections in order: **Card Details → Status → Goal Description → Resources → Prerequisites → Subtasks → Progress Notes → Completion Notes → Reflection & Lessons Learned → Unlocks**. Do not add, remove, or reorder sections — except for the dynamic-card Reflection & Lessons Learned placement documented under Subtask Tree Retention below.

### Key Fields

- **Current Status**: One of `Not Started`, `Preparing`, `Deferred`, `In Progress`, `Complete`
  - `Deferred` = intentionally postponed; card was previously active or preparing but deliberately paused pending better timing, prerequisites, or circumstances
- **Color Tier / Difficulty**: Determined by card value — 2-4 = 🟢 Green/Foundational, 5-7 = 🔵 Blue/Moderate, 8-9 = 🟡 Yellow/Substantial, 10-J = 🟠 Orange/Advanced, Q-K-A = 🔴 Red/Elite
- **Subtasks**: Use `- [x]` / `- [ ]` / `- [/]` / `- [-]` checkboxes (`[x]` = complete, `[ ]` = not started, `[/]` = in progress, `[-]` = skipped)
- **Prerequisites**: Reference other cards that must be completed first
- **Unlocks**: Cards that become available after this one is completed

### Subtask Tree Retention

- **Static cards** (status `Complete`) retain their full subtask tree permanently, as a record of how the goal was completed.
- **Dynamic cards** — cards with open-ended, recurring subtask groups rather than a fixed linear curriculum (e.g., joker-1, 10-hearts, ace-hearts) — prune closed subtask entries from the tree once they've been logged in CHANGELOG.md, to keep the file navigable. Course/book cards with a bounded curriculum are not dynamic in this sense and retain their full checklist regardless of status.
- Because dynamic cards don't hold a full history in-file, PROGRESS.md's On the Board table may track them by a different signal than a raw subtask fraction (e.g., joker-1 tracks Prerequisite completion instead — see its Prerequisites section; 10-hearts and ace-hearts track credit units/count instead).
- **[2026-08-10] Dynamic-card section order** — On dynamic cards (currently 09-clubs, joker-1, 10-hearts, ace-hearts), Reflection & Lessons Learned moves before Completion Notes: **Progress Notes → Reflection & Lessons Learned → Completion Notes → Unlocks**. Reflections on a dynamic card accumulate throughout its active life (one dated sub-entry per reflection-worthy session, newest first, per the file-wide dated-entry order) rather than being written once at the end, so the section reads oddly wedged after the one-time terminal Completion Notes. Bounded cards (course/book curricula) keep the original order, since their Reflection block is typically filled once, near completion.

### Suit Domains

- ♦️ Diamonds = Tools, Platforms & Certifications
- ♣️ Clubs = Applied Practice & Programming
- ♠️ Spades = Theory, Fundamentals & Mathematics
- ♥️ Hearts = Personal Development

## When Updating Cards

1. Always update the card file itself with the relevant changes (status, subtasks, progress notes, etc.)
2. Add a dated entry to `CHANGELOG.md` under the current date header (create one if it doesn't exist for today)
3. Review `PROGRESS.md` for staleness after any change that could affect it — status transitions, subtask edits, suit/tier movement — not just transitions to `Complete`. When the status change is to `Complete`, update counts and milestone checkboxes directly. For other changes, flag the specific stale figures (Summary counts, Completion by Suit/Tier tables, On the Board table) and confirm before editing rather than auto-correcting silently
4. Progress Notes entries should be prefixed with a bracketed date (e.g., `- [2026-02-17] Completed Chapter 3`)
5. If a card has a subdirectory (e.g., `05-hearts/`), supplementary materials like matrices or trackers live there alongside the card file
6. For non-standard or non-routine changes — a larger structural/strategic decision, a revised approach, or a cross-cutting realization — prompt whether it warrants a REFLECTIONS.md entry (cross-cutting insight) or a card Progress Notes entry (card-specific but noteworthy) — don't let it be logged only in CHANGELOG.md and left there. Routine updates (status/subtask edits, annotation refreshes, progress logging) don't need this prompt even when several cards are touched in one session.

## Changelog Conventions

**Entry header order** — When a single day's header covers multiple events, list them in descending order of significance: major milestones and status transitions first, routine progress updates last. Example: `Post-Semester Retro; Status Transitions; joker-2 apparel closed`.

**Annotation update entries** — Use the format `- [card] — [subtask] annotation updated (current annotation)`. The parenthetical reflects the annotation's current state after the update, not a before/after diff. Example: `- [joker-1] — [Optimize Github profile] annotation updated (pinned repositories, repository descriptions)`.

**Subsection order** — Within a single entry, `###` subsections (`Added`/`Changed`/`Completed`/`Removed`/`Notes`/`Process`) are ordered chronologically by when the changes occurred during the session — earliest change at the top, most recent at the bottom — not by a fixed Added→Changed→Completed sequence. This mirrors the entry-header convention at the file level: entries themselves run reverse-chronological (newest date first), but the content within a single entry runs chronological (oldest action first).

## System Principles

Cross-cutting rules of thumb for evaluating scope and structure, distinct from the dated, event-motivated calls in Standing Decisions below.

- **Opportunity cost as scope guard** — When evaluating whether to add a subtask to an existing card or spin it into a new backlog entry, the primary criterion is opportunity cost — what is lost by *not* including it, rather than whether it fits. Pure ornamentation has no meaningful opportunity cost for exclusion. This principle applies across all suits.

## Standing Decisions

Cross-cutting decisions that apply across cards and sessions, not tied to any single card's progress.

- **[2026-08-19] Codebase documentation is part of the active session** — Do not push progress notes, card updates, or CHANGELOG entries to a separate end-of-day commit window. Document within the session itself while context is fresh. End-of-day Claude Code sessions are for batching multiple cards only when sessions genuinely overlap — not as a default deferral mechanism.
- **[2026-06-14] LinkedIn content strategy** — Personal/lifestyle milestones (e.g., weight loss progress) are excluded from individual LinkedIn posts. Reserved for a future cumulative progress-arc reflection post.
- **[2026-06-16] Subtask count in changelog** — Do not log subtask count deltas (e.g., `17/51 → 17/54`) in CHANGELOG.md. The count is represented in the card itself; logging it in the changelog is redundant. Omit the `#### Process` section entirely if it has nothing else to say.
- **[2026-07-14] Prerequisite/accelerant terminology** — When a Prerequisites entry or BACKLOG dependency note describes optional scaffolding that speeds up or strengthens a goal but isn't required to achieve it, describe it as a supporting goal or accelerant rather than framing it with "prerequisite"/"requirement" language that implies a hard gate. Reserve "prerequisite"/"requirement" for genuine hard dependencies (visa rules, degree requirements for a specific posting, curriculum ordering). Motivated by a diagnosed pattern where gating language caused a goal to be avoided because it was modeled as more blocked and laborious than it actually was.
- **[2026-08-03] Dated entry order, file-wide** — Any dated/note-type entry list (Progress Notes, standing logs, etc.) runs reverse-chronological, newest first, matching CHANGELOG.md's convention. New entries go at the top of their list, not appended at the bottom. This overrules any future prompt instruction to append at the bottom.
