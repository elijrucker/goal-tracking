# Resources

Master reference list of all courses, books, platforms, and links associated with the 52-card goal tracking system. Organized by suit and card.

---

## ♦️ Diamonds - Tools, Platforms & Certifications

### 2♦️ Markdown

- [Markdown Guide](https://www.markdownguide.org) - Comprehensive markdown reference
- [VS Code Markdown Support](https://code.visualstudio.com/docs/languages/markdown) - VS Code markdown documentation

### 3♦️ Python Crash Course

- Python Crash Course by Eric Matthes (book)

### 4♦️ Foundational C# with Microsoft

- [FreeCodeCamp - Foundational C# with Microsoft](https://www.freecodecamp.org/learn/foundational-c-sharp-with-microsoft) - Official cert course (50% complete)

### 5♦️ Anki - Python

- [Anki](https://apps.ankiweb.net) - Spaced repetition flashcard platform
- Search AnkiWeb for Python decks

### 6♦️ Docker Terminal/CLI

- [Docker CLI Documentation](https://docs.docker.com/engine/reference/commandline/cli/) - Official reference

### 7♦️ CI/CD Toolchain

- [Bitbucket Documentation](https://support.atlassian.com/bitbucket-cloud/)
- [Bamboo Documentation](https://confluence.atlassian.com/bamboo)
- [Octopus Deploy Documentation](https://octopus.com/docs)

### 8♦️ Anki - C#

- [Anki](https://apps.ankiweb.net) - Spaced repetition flashcard platform
- Search AnkiWeb for C# decks

### 9♦️ Boot.dev

- [Boot.dev](https://www.boot.dev) - Backend development learning platform

### 10♦️ Kubernetes

- [LinkedIn Learning - Getting Started with Kubernetes](https://www.linkedin.com/learning/paths/getting-started-with-kubernetes?u=1009514) - 9-hour learning path

### J♦️ AWS Skill Builder Learning Plan

- [AWS Skill Builder - Developer Learning Plan](https://skillbuilder.aws/search?la=cta&cta=topbanner&searchText=developer-learning-plan) - Official AWS training

### Q♦️ CS50M

- [CS50M - Mobile App Development with React Native](https://cs50.harvard.edu/mobile/) - Harvard/edX course

### K♦️ AWS DVA Prep Course

- [Stephane Maarek - AWS DVA-C02](https://www.udemy.com/course/aws-certified-developer-associate-dva-c01/) - Primary option
- [FreeCodeCamp - Andrew Brown DVA-C02](https://www.freecodecamp.org/news/ultimate-aws-certified-developer-associate-dva-c02-course-from-andrew-brown/) - Alternative option

### A♦️ AWS Certified Developer Associate

- [AWS Certified Developer Associate](https://aws.amazon.com/certification/certified-developer-associate/) - Official certification page

---

## ♣️ Clubs - Applied Practice & Programming

### 2♣️ FCC Algorithms & Data Structures

- [FreeCodeCamp - Algorithms and Data Structures](https://www.freecodecamp.org/learn/javascript-algorithms-and-data-structures/) - 6-hour course

### 3♣️ Pragmatic Programmer

- The Pragmatic Programmer by David Thomas & Andrew Hunt (book)

### 4♣️ Clean Code

- Clean Code by Robert C. Martin (book) - Already owned

### 5♣️ CS50W

- [CS50W - Web Programming with Python and JavaScript](https://cs50.harvard.edu/web/) - Harvard/edX course

### 6♣️ Algorithms with Python

- TBD - Select resource after completing Python Crash Course

### 7♣️ Intermediate Web Projects

- Reference: 40 Project Ideas list (see DECK_OVERVIEW.md)
- Suggested: Weather App, Portfolio Website, Chat App

### 8♣️ Google Tech Dev Guide

- [Google Tech Dev Guide](https://techdevguide.withgoogle.com) - Official Google resource

### 9♣️ LeetCode Practice

- [LeetCode](https://leetcode.com) - Algorithm and interview practice platform

### 10♣️ API Development Project

- TBD - Select framework after completing CS50W (Django, Flask, FastAPI, etc.)

### J♣️ Advanced/Clone Projects

- Reference: 40 Project Ideas list (see DECK_OVERVIEW.md)
- Suggested: Netflix Clone, Instagram Clone, Social Media Dashboard

### Q♣️ Full-Stack Application

- TBD - Define stack after completing CS50W and intermediate projects

### K♣️ Testing/TDD Milestone

- TBD - Select course or book (options: Test Driven Development by Example, pytest documentation)

### A♣️ Open Source Contribution

- [GitHub Explore](https://github.com/explore) - Find open source projects
- [First Contributions](https://firstcontributions.github.io) - Guide for first-time contributors
- [Good First Issues](https://goodfirstissues.com) - Curated beginner-friendly issues

---

## ♠️ Spades - Theory, Fundamentals & Mathematics

### 2♠️ CS50x

- [CS50x - Introduction to Computer Science](https://cs50.harvard.edu/x/) - Harvard/edX course (in progress)

### 3♠️ CS Distilled

- Computer Science Distilled by Wladston Ferreira Filho (book)

### 4♠️ CS50 Cybersecurity

- [CS50 Cybersecurity](https://cs50.harvard.edu/cybersecurity/) - Harvard/edX course

### 5♠️ Discrete Mathematics

- TBD - Select textbook or course (options: MIT OpenCourseWare, Discrete Mathematics and Its Applications by Rosen)

### 6♠️ CS50 AI

- [CS50 AI - Introduction to Artificial Intelligence with Python](https://cs50.harvard.edu/ai/) - Harvard/edX course

### 7♠️ Computer Architecture

- TBD - Select course (options: Nand2Tetris, MIT OpenCourseWare 6.004)

### 8♠️ Operating Systems

- TBD - Select course (options: MIT OpenCourseWare 6.828, Operating Systems: Three Easy Pieces - free online)

### 9♠️ Computer Systems: A Programmer's Perspective

- Computer Systems: A Programmer's Perspective by Bryant & O'Hallaron (book)

### 10♠️ MIT Single Variable Calculus

- [MIT OpenCourseWare - Single Variable Calculus](https://ocw.mit.edu/courses/18-01sc-single-variable-calculus-fall-2010/) - Free course

### J♠️ MIT Mathematics for Computer Science

- [MIT OpenCourseWare - Mathematics for Computer Science](https://ocw.mit.edu/courses/6-042j-mathematics-for-computer-science-fall-2010/) - Free course

### Q♠️ Structure & Interpretation of Computer Programs (SICP)

- [SICP - Free Online](https://mitpress.mit.edu/sites/default/files/sicp/full-text/book/book.html) - Free MIT Press edition
- Structure and Interpretation of Computer Programs by Abelson & Sussman (book)

### K♠️ Introduction to Algorithms (CLRS)

- Introduction to Algorithms by Cormen, Leiserson, Rivest & Stein (book)

### A♠️ The Art of Computer Programming (Knuth)

- The Art of Computer Programming (Vol. 1-4) by Donald Knuth (book series)

---

## ♥️ Hearts - Personal Development

### 2♥️ Index Investing Fundamentals

**Primary:**

- The Little Book of Common Sense Investing by John Bogle (book) — foundational index investing philosophy
- [Bogleheads Forum](https://www.bogleheads.org) - Index investing community
- [European Bogleheads Wiki](https://www.bogleheads.org/wiki/Investing_from_Europe) - EU-specific guidance

**Fundamental & Value Analysis:**

- [Damodaran - Foundations of Finance](https://pages.stern.nyu.edu/~adamodar/New_Home_Page/webcastfoundationsonline.htm) - NYU Stern, free — ~3-4 hrs
- [Damodaran - Investment Philosophies](https://pages.stern.nyu.edu/~adamodar/) - NYU Stern, free — selected modules

**Technical Analysis:**

- [IG Academy - Technical Analysis](https://www.ig.com/en/learn-trading/ig-academy) - FCA-regulated, free — ~4-5 hrs

**Options:**

- [Options Industry Council](https://www.optionseducation.org) - OIC, free — income strategy track ~5-10 hrs

**Futures:**

- [CME Institute - Introduction to Futures](https://www.cmegroup.com/education.html) - CME Group, free — ~3-6 hrs

**Forex:**

- [BabyPips - School of Pipsology](https://www.babypips.com/learn/forex) - free — ~6-8 hrs core

**Global Markets Context:**

- [Yale Financial Markets - Robert Shiller](https://www.coursera.org/learn/financial-markets-global) - Coursera, free to audit

### 3♥️ Global Markets & Economics

- [Yale Financial Markets - Robert Shiller](https://www.coursera.org/learn/financial-markets-global) - Free on Coursera
- [Financial Times](https://www.ft.com) - International financial news

### 4♥️ Growth Investing Strategy

- One Up On Wall Street by Peter Lynch (book)
- Common Stocks and Uncommon Profits by Philip Fisher (book)

### 5♥️ Weight Loss: 25 lbs

- TBD - Track via preferred health app or journal

### 6♥️ South American Markets Investment ($5K)

- TBD - Select brokerage and regional ETF/fund (e.g., iShares Latin America ETF)

### 7♥️ Weight Loss: 50 lbs

- TBD - Track via preferred health app or journal

### 8♥️ Southeast Asian Markets Investment ($10K)

- TBD - Select brokerage and regional ETF/fund (e.g., iShares MSCI Southeast Asia ETF)
- Target markets: Vietnam, Thailand, Philippines, Indonesia

### 9♥️ Navy SEAL Intermediate Fitness Standards

- Unbeatable Mind / Navy SEAL Fitness by Mark de Lisle (book)

### 10♥️ Complete Associate's Degree

- Current institution - track via academic portal

### J♥️ Weight Loss: 75 lbs

- TBD - Track via preferred health app or journal

### Q♥️ Polish C1 Certification

- [FSI Polish Fast](https://fsi-languages.yojik.eu/languages/Polish.html) - Primary learning resource
- [CEFR C1 Polish Certification](https://certyfikatpolski.pl/en/) - Official Polish language certification

### K♥️ Eastern European Markets Investment ($15K via EU)

- TBD - Select EU-based brokerage after relocation (e.g., DEGIRO, XTB)
- Target: Warsaw Stock Exchange (GPW) and Eastern European ETFs
- Note: Requires EU residency - complete after relocation to Berlin/Poland

### A♥️ Complete Bachelor's Degree

- TBD - Confirm institution and program after Associate's completion

### Backlog (Hearts — Unassigned)

- [Poland Business Harbour](https://www.gov.pl/web/poland-businessharbour-en) - Polish government IT sector relocation program; tax reliefs for R&D/innovation, business setup support, and "Why Poland" reference material. Relevant to pre-relocation and cross-border tax strategy cards.

---

## 🃏 Jokers

### Joker 1: Software Developer Job Offer in Berlin

- [Berlin Tech Job Boards](https://berlinstartupjobs.com) - Berlin-specific tech jobs
- [LinkedIn Jobs](https://www.linkedin.com/jobs/) - General job search
- [Stack Overflow Jobs](https://stackoverflow.com/jobs) - Developer-focused job board

### Joker 2: Own a Sport Luxury Vehicle

- Resources TBD when goal is defined

---

## General Reference

### Learning Platforms

- [MIT OpenCourseWare](https://ocw.mit.edu) - Free university courses
- [Coursera](https://www.coursera.org) - Online courses
- [edX](https://www.edx.org) - Online courses (hosts CS50 series)
- [FreeCodeCamp](https://www.freecodecamp.org) - Free coding courses and certifications
- [LinkedIn Learning](https://www.linkedin.com/learning/) - Professional development courses

### Developer Tools

- [GitHub](https://www.github.com) - Version control and open source
- [VS Code](https://code.visualstudio.com) - Primary development environment
- [Anki](https://apps.ankiweb.net) - Spaced repetition flashcards

### EU Investing Resources

- [DEGIRO](https://www.degiro.eu) - Low-cost EU broker
- [XTB](https://www.xtb.com) - EU broker with Polish origins
- [justETF](https://www.justetf.com) - European ETF screener and research
