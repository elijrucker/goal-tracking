...
- Professional validation of self-taught skills
...
- [/] Ensure 3 strong portfolio projects are live and documented
  - [ ] Project 3 — Java workout tracking app (pivoted 2026-08-28 from Java CLI copy tool). Scope narrowed 2026-09-07 to the MVP — the portfolio requirement is satisfied by MVP alone; later phases moved to BACKLOG.md as optional extensions. MVP interface revised 2026-09-16 to web-based; the original mobile-first framing now applies to a deferred mobile frontend (BACKLOG.md), not the MVP itself.
    ...
    - [ ] Architecture reference (MATC javaProgramming instructor feedback, 2026-08-28): staged technical progression, not a timeline gate — (1) backend design/build feasible now with current coursework, (2) DB connection achievable with Advanced Java-level knowledge, (3) full best-practices stack (Maven dependency management, Hibernate ORM, unit tests, AWS deployment, Cognito auth, robust API produce/consume) matches Enterprise Java-level knowledge. Independent early adoption of later-stage practices explicitly endorsed by instructor, not required.
